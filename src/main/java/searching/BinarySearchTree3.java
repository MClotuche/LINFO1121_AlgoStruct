...
public class BinarySearchTree3 {
    ...
    public static Integer ceil(BSTNode<Integer> root, int value) {
        ...
        int cmp = root.getKey().compareTo(value);
        if (cmp ==0)return root.getKey();
        if (cmp<0){//val < k
            return ceil(root.right,value);
        }
        else { //val > k
            Integer actualBest = root.getKey();
            Integer concurrent = ceil(root.left,value);
            if (concurrent == null || concurrent>actualBest)return actualBest;
